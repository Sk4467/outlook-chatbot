...
from .pandasai_integration import is_analysis_question, analyze_with_pandasai, get_structured_data_summary
...
async def ask(req: AskRequest) -> Dict[str, Any]:
    ...
    # Check if this is an analysis question that should use PandasAI
    if route == "attachment_tabular" and is_analysis_question(req.question):
        print(f"[PANDASAI] Analysis question detected: {req.question[:50]}...")
        idx_hits = topk_from_collection(ATTACHMENTS_TABULAR_IDX, req.question, k=min(3, req.k))
        
        if not idx_hits:
            print("[PANDASAI] No index hits")
            return {"answer": "No relevant tabular attachments found for analysis.", "route": "pandasai_no_data", "sources": []}
        
        print(f"[PANDASAI] Found {len(idx_hits)} data sources")
        specs = []
        for i, h in enumerate(idx_hits):
            ...
            print(f"[PANDASAI] {i+1}: {filename}")
        
        try:
            tables = load_dataframes(specs)
            print(f"[PANDASAI] Loaded {len(tables)} tables")
            
            # Use PandasAI for analysis
            analysis_result = analyze_with_pandasai(req.question, tables)
            
            # If PandasAI is unavailable, fall back to enhanced analysis
            if analysis_result.get("analysis_type") == "pandasai_unavailable":
                print(f"[PANDASAI] Falling back to enhanced analysis")
                fallback_result = answer_with_pandasai(req.question, tables)
                
                sources = [{"filename": s["filename"], "sheet": s.get("sheet")} for s in specs]
                return {
                    "answer": f"**Note:** PandasAI not available - using enhanced analysis instead.\n\n{fallback_result.get('answer', '')}",
                    "route": "pandasai_fallback",
                    "sources": sources,
                    "tables_used": fallback_result.get("tables_used", []),
                    "analysis_type": "enhanced_fallback",
                    "install_suggestion": "Install PandasAI with: pip install pandasai[excel]"
                }
            
            sources = [{"filename": s["filename"], "sheet": s.get("sheet")} for s in specs]
            return {
                "answer": analysis_result.get("answer", ""),
                "route": "pandasai_analysis",
                "sources": sources,
                "tables_used": analysis_result.get("tables_used", []),
                "analysis_type": analysis_result.get("analysis_type", "pandasai"),
                "tables_analyzed": analysis_result.get("tables_analyzed", 0)
            }
            
        except Exception as e:
            print(f"[PANDASAI] ERROR: {str(e)} - falling back to enhanced analysis")
            ...
            try:
                ...
                sources = [{"filename": s["filename"], "sheet": s.get("sheet")} for s in specs]
                return {
                    "answer": f"**Note:** PandasAI failed - using enhanced analysis.\n\n{fallback_result.get('answer', '')}",
                    "route": "pandasai_fallback_error",
                    "sources": sources,
                    "tables_used": fallback_result.get("tables_used", []),
                    "analysis_type": "enhanced_fallback_error",
                    "original_error": str(e)
                }
            except Exception as fallback_error:
                return {
                    "answer": f"Both PandasAI and enhanced analysis failed. PandasAI error: {str(e)}. Enhanced error: {str(fallback_error)}",
                    "route": "total_failure",
                    "sources": [],
                    "error": str(e),
                    "analysis_type": "total_failure"
                }
