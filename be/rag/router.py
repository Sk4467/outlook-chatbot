...
def _classify_intent_heuristic(question: str) -> Tuple[Route, float]:
    ...
    # Enhanced PDF/document detection
    pdf_keywords = [
        "attachment", "pdf", "document", "paper", "research", "study", "framework", 
        "methodology", "approach", "algorithm", "code", "translation", "conversion",
        "agentic", "objective", "purpose", "goal", "implementation", "technical"
    ]
    if any(keyword in q for keyword in pdf_keywords):
        return "attachment_semantic", 0.7
...
def _classify_intent_llm(question: str) -> Tuple[Route, float]:
    ...
    guideline = (
        ...
        "- mail_body_semantic: The answer is likely found in the email body text (payment info, notifications, etc.).\n"
        "- attachment_tabular: The question requires numeric analysis, aggregation, filtering, or spreadsheet/CSV data operations (sales data, counts, averages, etc.).\n"
        "- attachment_semantic: The answer is found in documents, PDFs, research papers, or text attachments. Questions about frameworks, methodologies, objectives, technical content, algorithms, code translation, research findings, or document content.\n"
        "- unknown: Cannot determine or unclear.\n\n"
        "Examples:\n"
        "- 'What is the objective of the agentic framework?' → attachment_semantic\n"
        "- 'How many sales in Asia?' → attachment_tabular\n"
        "- 'Payment details from Cursor' → mail_body_semantic\n\n"
