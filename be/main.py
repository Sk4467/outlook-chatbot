...
class BulkIngestRequest(BaseModel):
    ...
    ingest_mail_bodies: bool = True


@app.post("/gmail/bulk-fetch-ingest")
def bulk_fetch_and_ingest(req: BulkIngestRequest):
    """
    Fetch emails and automatically ingest all mail bodies and attachments into vector DB.
    This replaces the cumbersome multi-step process with a single operation.
    """
    if req.user not in (1, 2):
        return {"error": "User must be 1 or 2"}
    
    email_account = os.getenv(f"GMAIL_USER{req.user}", "")
    email_password = os.getenv(f"GMAIL_PASS{req.user}", "")
    if not email_account or not email_password:
        return {"error": f"Missing GMAIL_USER{req.user}/GMAIL_PASS{req.user} env vars"}
    
    try:
        print(f"[BULK INGEST] Starting bulk fetch and ingest for user {req.user}, last {req.n} emails")
        
        # Step 1: Fetch emails
        emails = gmail_fetch_emails(email_account, email_password, n=req.n)
        print(f"[BULK INGEST] Fetched {len(emails)} emails")
        
        results = {
            "emails_fetched": len(emails),
            "mail_bodies_ingested": 0,
            ...
            if req.ingest_mail_bodies and email.get("body_text"):
                try:
                    ...
                    if ingest_result.get("chunks", 0) > 0:
                        ...
                        print(f"[BULK INGEST] ✅ Mail body ingested: {ingest_result.get('chunks', 0)} chunks")
                    else:
                        email_result["errors"].append("Mail body ingestion returned 0 chunks")
                except Exception as e:
                    error_msg = f"Mail body ingestion failed: {str(e)}"
                    email_result["errors"].append(error_msg)
                    results["errors"].append(f"Email {email_id}: {error_msg}")
                    print(f"[BULK INGEST] ❌ {error_msg}")
            
            # Step 3: Process attachments if requested
            if req.ingest_attachments:
                try:
                    # Get attachments list
                    attachments = gmail_list_attachments(email_account, email_password, email_id)
                    print(f"[BULK INGEST] Found {len(attachments)} attachments")
                    ...
                    for attachment in attachments:
                        ...
                        try:
                            # Download attachment
                            attachment_data, content_type, filename = gmail_fetch_attachment_bytes(
                                email_account, email_password, email_id, attachment["index"]
                            )
                            
                            # Create blob URL (simulate the existing structure)
                            blob_url = f"/gmail/message/{email_id}/attachments/{attachment['index']}/download?user={req.user}"
                            
                            # Cache the attachment data for RAG ingestion
                            from rag.data_cache import cache_blob_data
                            cache_blob_data(blob_url, filename or "unknown", attachment_data)
                            
                            # Ingest attachment using direct data processing
                            from rag.api import _ingest_attachment_bytes_core
                            ingest_result = _ingest_attachment_bytes_core(
                                data=attachment_data,
                                message_id=email_id,
                                attachment_id=f"{email_id}_{attachment['index']}",
                                filename=filename or attachment.get("filename", "unknown"),
                                content_type=content_type or attachment.get("content_type", ""),
                                blob_uri=blob_url
                            )
                            
                            if ingest_result.get("chunks", 0) > 0:
                                attachment_result["ingested"] = True
                                results["attachments_ingested"] += 1
                                print(f"[BULK INGEST] ✅ Attachment ingested: {filename} ({ingest_result.get('chunks', 0)} chunks)")
                            else:
                                attachment_result["error"] = "Ingestion returned 0 chunks"
                                print(f"[BULK INGEST] ⚠️ Attachment {filename} returned 0 chunks")
                            
                        except Exception as e:
                            error_msg = f"Attachment processing failed: {str(e)}"
                            attachment_result["error"] = error_msg
                            results["errors"].append(f"Email {email_id}, attachment {attachment.get('filename')}: {error_msg}")
                            print(f"[BULK INGEST] ❌ {error_msg}")
                        
                        email_result["attachments"].append(attachment_result)
                        results["attachments_processed"] += 1
                
                except Exception as e:
                    error_msg = f"Attachment listing failed: {str(e)}"
                    email_result["errors"].append(error_msg)
                    results["errors"].append(f"Email {email_id}: {error_msg}")
                    print(f"[BULK INGEST] ❌ {error_msg}")
            
            results["processed_emails"].append(email_result)
        
        print(f"[BULK INGEST] ✅ Completed! Bodies: {results['mail_bodies_ingested']}, Attachments: {results['attachments_ingested']}")
        return results
        
    except Exception as e:
        error_msg = f"Bulk fetch and ingest failed: {str(e)}"
        print(f"[BULK INGEST] ❌ {error_msg}")
