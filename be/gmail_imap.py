...
def list_attachments(
    ...
    try:
        ...
        # Handle message_id format properly for IMAP
        try:
            # Try as-is first (might be sequence number)
            status, msg_data = imap.fetch(message_id, "(RFC822)")
        except Exception:
            # If that fails, try encoding to bytes
            try:
                status, msg_data = imap.fetch(message_id.encode("ascii"), "(RFC822)")
            except Exception:
                # Last resort - try as string with quotes
                status, msg_data = imap.fetch(f'"{message_id}"', "(RFC822)")
